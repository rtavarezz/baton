...
var Migration001InitDatabase = &migrate.Migration{
	...
	Up: []string{`
		...
		CREATE TABLE IF NOT EXISTS ` + vars.TableExecutionPayload + ` (
			...
			PRIMARY KEY (id, chain_id, is_tob, proposer_pubkey, block_hash)  -- Composite primary key
		);

		CREATE UNIQUE INDEX IF NOT EXISTS ` + vars.TableExecutionPayload + `_slot_pk_hash_idx ON ` + vars.TableExecutionPayload + `(slot, proposer_pubkey, block_hash, chain_id, is_tob);
		...
		CREATE INDEX IF NOT EXISTS ` + vars.TableBuilderBlockSubmission + `_chain_id_idx ON ` + vars.TableBuilderBlockSubmission + `("chain_id");
		CREATE INDEX IF NOT EXISTS ` + vars.TableBuilderBlockSubmission + `_is_tob_idx ON ` + vars.TableBuilderBlockSubmission + `("is_tob");
